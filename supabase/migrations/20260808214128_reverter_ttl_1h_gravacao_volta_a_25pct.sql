-- Recuperada do transcript da sessão (C--Users-PC/61196a62-f019-4dc1-846d-81415e5b5176.jsonl, 2026-08-08T21:41:23.496Z) em 14/09/2026 —
-- a migration foi aplicada em produção pela ferramenta MCP apply_migration, que registrou a
-- versão 20260808214128 sem gravar arquivo no repositório (MF-AUD-058). Conteúdo abaixo é o que
-- foi enviado ao banco naquela chamada.

-- TTL de 1h revertido (ver comentario em _shared/ai/prompt.ts). A gravacao volta a custar
-- +25% da entrada, nao +100%.
...
with precos as (
  select
    ...
    0.50::numeric  as usd_adicional_gravacao_por_milhao
),
base as (
  select
