...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!instanceId || !token) {
      console.error("WhatsApp credentials not configured. Cannot send scheduled messages.");
      return new Response(JSON.stringify({
        success: false,
        sent: 0,
        error: "Credenciais WhatsApp não configuradas em app_settings.",
      }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }

    const provider = createWhatsAppProvider({ instanceId, token, clientToken });
    ...
    for (const order of (orders || [])) {
      ...
      const originalPhone = normalizePhoneNumber(
      ...
      try {
        const sendResult = await provider.sendText(targetPhone, message);
