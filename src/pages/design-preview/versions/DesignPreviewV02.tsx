...
/* ─────────────────────────────────────────────────────────────────────────────
   Marineflow ERP — Design Preview V02 (Fase 1.4)
   Identidade HBR Systems mais presente. Evolução da V01.
   Snapshot preservado. Não altere este arquivo após aprovação.
   V02 — Identidade HBR Systems · linguagem náutica · tokens isolados.
───────────────────────────────────────────────────────────────────────────── */

/* ── Paleta V02 (inline, não afeta tokens globais) ───────────────────────── */
const V2 = {
  hdrBg:   '#020E1A',   /* navy mais profundo que navy-950 */
  barBg:   '#010B13',
  paper:   '#F7F6F2',   /* fundo quente — mais sóbrio que FAFAF7 */
  gold:    '#C8A24B',
  goldDim: 'rgba(200,162,75,0.45)',
} as const;

/* ── Dados mock — mesmos da V01 ──────────────────────────────────────────── */
const OS_ROWS: OSRow[] = [
...
/* ════════════════════════════════════════════════════════════════════════════
   COMPONENTE PRINCIPAL — V02
════════════════════════════════════════════════════════════════════════════ */
export default function DesignPreviewV02() {
  return (
    <div style={{ fontFamily: 'var(--mf-font-sans)', background: V2.paper, color: 'var(--mf-ink-900)', minHeight: '100vh', fontSize: 'var(--mf-text-base)', lineHeight: 1.5 }}>

      {/* ── Header HBR Systems ──────────────────────────────────────────── */}
      <div style={{ position: 'sticky', top: 0, zIndex: 40, background: V2.hdrBg }}>
        <div style={{ height: 52, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 20px' }}>

          {/* Marca tipográfica HBR Systems + produto */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            {/* HBR / SYSTEMS */}
            <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', paddingRight: 14, borderRight: '1px solid rgba(255,255,255,0.07)', lineHeight: 1 }}>
              <span style={{ fontWeight: 800, fontSize: 20, letterSpacing: '-0.03em', color: '#ffffff', fontFamily: 'var(--mf-font-sans)', lineHeight: 1 }}>HBR</span>
              <span style={{ fontSize: 7, fontWeight: 700, letterSpacing: '0.22em', color: V2.gold, textTransform: 'uppercase', marginTop: 3, lineHeight: 1 }}>SYSTEMS</span>
            </div>
            {/* Nome do produto + slogan */}
            <div>
              <div style={{ color: 'var(--mf-ink-100)', fontWeight: 600, fontSize: 'var(--mf-text-sm)', letterSpacing: '-0.01em', lineHeight: 1.2 }}>Marineflow ERP</div>
              <div style={{ color: V2.goldDim, fontSize: 9, letterSpacing: '0.04em', lineHeight: 1.2, marginTop: 2 }}>Energia. Controle. Confiança.</div>
            </div>
          </div>

          {/* Nav de seções + links de versão + badges */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            {/* Seções */}
            <nav style={{ display: 'flex', gap: 1 }}>
              {NAV_SECTIONS.map(n => (
                <a key={n.id} href={`#${n.id}`}
                  style={{ color: 'var(--mf-ink-500)', fontSize: 11, padding: '3px 7px', borderRadius: 3, textDecoration: 'none', fontWeight: 500 }}
                  onMouseEnter={e => { const el = e.target as HTMLElement; el.style.color = V2.gold; el.style.background = 'rgba(255,255,255,0.04)'; }}
                  onMouseLeave={e => { const el = e.target as HTMLElement; el.style.color = 'var(--mf-ink-500)'; el.style.background = 'transparent'; }}>
                  {n.label}
                </a>
              ))}
            </nav>
            {/* Links de versão */}
            <div style={{ display: 'flex', gap: 6, paddingLeft: 10, borderLeft: '1px solid rgba(255,255,255,0.07)' }}>
              <a href="/design-preview-v01" style={{ color: 'var(--mf-ink-600)', fontSize: 10, textDecoration: 'none', fontWeight: 500, padding: '2px 6px', borderRadius: 3, border: '1px solid rgba(255,255,255,0.06)' }}>← V01</a>
              <a href="/design-preview-compare" style={{ color: 'var(--mf-ink-600)', fontSize: 10, textDecoration: 'none', fontWeight: 500, padding: '2px 6px', borderRadius: 3, border: '1px solid rgba(255,255,255,0.06)' }}>Comparativo</a>
            </div>
            {/* Badges */}
            <div style={{ display: 'flex', gap: 5 }}>
              <span style={{ background: `rgba(200,162,75,0.12)`, color: V2.gold, fontSize: 10, fontWeight: 700, padding: '2px 7px', borderRadius: 3, border: `1px solid rgba(200,162,75,0.22)`, letterSpacing: '0.06em' }}>V02</span>
              <span style={{ background: 'rgba(255,255,255,0.04)', color: 'var(--mf-ink-500)', fontSize: 10, fontWeight: 600, padding: '2px 7px', borderRadius: 3, border: '1px solid rgba(255,255,255,0.07)', letterSpacing: '0.04em' }}>Design Lab</span>
            </div>
          </div>
        </div>

        {/* Linha fluida — acento dourado HBR Systems */}
        <div style={{ height: 2, background: `linear-gradient(90deg, ${V2.hdrBg} 0%, rgba(200,162,75,0.25) 15%, rgba(200,162,75,0.75) 40%, #C8A24B 50%, rgba(200,162,75,0.75) 60%, rgba(200,162,75,0.25) 85%, ${V2.hdrBg} 100%)` }} />
      </div>

      {/* ── Barra de contexto ───────────────────────────────────────────── */}
      <div style={{ background: V2.barBg, padding: '5px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--mf-ink-600)' }}>Design Lab</span>
          <span style={{ color: 'var(--mf-ink-700)', fontSize: 'var(--mf-text-xs)' }}>—</span>
          <span style={{ fontSize: 'var(--mf-text-xs)', color: 'var(--mf-ink-600)' }}>Snapshot visual · sem dados reais · feat/ui-design-phase-1-preview</span>
        </div>
        <a href="/design-preview-compare" style={{ fontSize: 'var(--mf-text-xs)', color: 'var(--mf-navy-400)', textDecoration: 'none', fontWeight: 500 }}>Comparativo de versões ↗</a>
      </div>
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '20px 20px 48px' }}>
        ...
        {/* ── ATENÇÃO DE HOJE ────────────────────────────────────────────── */}
        <SectionHeader id="atencao" title="Atenção de hoje" sub="Itajaí Marine Center · sexta, 16 mai 2026" />

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8, marginBottom: 24 }}>
          <KpiCard label="OS abertas" value="19" detalhe="3 atrasadas" detalheColor="var(--mf-danger-600)" accent="danger" />
          <KpiCard label="Orçamentos pendentes" value="5" detalhe="1 vence hoje" detalheColor="var(--mf-warning-600)" accent="warning" />
          <KpiCard label="WhatsApp aguardando" value="2" detalhe="sem leitura há 42 min" detalheColor="var(--mf-info-600)" accent="info" />
          <KpiCard label="A receber esta semana" value="R$ 12.843" detalhe="2 vencidas ontem" detalheColor="var(--mf-danger-600)" accent="danger" />
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 5, marginBottom: 28 }}>
          {ALERTAS.map((a, i) => {
            ...
            return (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, background: c.bg, border: `1px solid ${c.border}`, borderLeft: `2px solid ${c.border}`, borderRadius: 4, padding: '6px 12px' }}>
                <span style={{ width: 15, height: 15, borderRadius: '50%', background: c.border, color: '#fff', display: 'grid', placeItems: 'center', fontSize: 9, fontWeight: 800, flexShrink: 0 }}>{c.icon}</span>
                <span style={{ fontSize: 'var(--mf-text-sm)', color: c.color, flex: 1 }}>{a.msg}</span>
                <button style={{ fontSize: 11, fontWeight: 600, color: c.color, background: 'transparent', border: `1px solid ${c.border}`, borderRadius: 3, padding: '2px 8px', cursor: 'pointer', whiteSpace: 'nowrap', fontFamily: 'var(--mf-font-sans)' }}>{a.acao}</button>
              </div>
            );
          })}
        </div>
        ...
        {/* ── FILA OPERACIONAL ───────────────────────────────────────────── */}
        <SectionHeader id="fila" title="Controle técnico — Ordens de Serviço" sub="19 registros · exibindo 7 · filtro: em aberto" />

        <div style={{ background: '#fff', border: '1px solid var(--mf-ink-100)', borderRadius: 5, overflow: 'hidden', marginBottom: 28 }}>
          {/* Toolbar */}
          <div style={{ padding: '7px 12px', borderBottom: '1px solid var(--mf-ink-100)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, background: V2.paper }}>
            <div style={{ display: 'flex', gap: 6, alignItems: 'center', flex: 1 }}>
              <input placeholder="Buscar por OS, cliente ou embarcação…" style={{ height: 28, padding: '0 8px', border: '1px solid var(--mf-ink-200)', borderRadius: 3, fontSize: 'var(--mf-text-sm)', color: 'var(--mf-ink-700)', background: '#fff', outline: 'none', width: 260, fontFamily: 'var(--mf-font-sans)' }} />
              <FilterChip active>Em aberto</FilterChip>
              <FilterChip>Alta prioridade</FilterChip>
              <FilterChip>Sem técnico</FilterChip>
              <FilterChip>Peça aguardando</FilterChip>
            </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            {/* Cabeçalho navy — ancora visual da operação */}
            <thead>
              <tr style={{ borderBottom: '1px solid var(--mf-navy-800)', background: 'var(--mf-navy-900)' }}>
                {['OS', 'Cliente', 'Embarcação', 'Serviço', 'Status', 'Prior.', 'Técnico', 'Valor', 'Prazo'].map(h => (
                  <th key={h} style={{ padding: '7px 9px', textAlign: 'left', fontSize: 10, fontWeight: 700, color: 'rgba(255,255,255,0.45)', letterSpacing: '0.07em', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>{h}</th>
                ))}
                <th style={{ padding: '7px 9px', width: 32 }} />
              </tr>
            </thead>
            <tbody>
              {OS_ROWS.map((row, i) => {
                ...
                return (
                  <tr key={row.num}
                    style={{ borderBottom: i < OS_ROWS.length - 1 ? '1px solid var(--mf-ink-50)' : undefined, background: critical ? 'var(--mf-danger-50)' : 'transparent', borderLeft: critical ? '2px solid var(--mf-danger-400)' : '2px solid transparent' }}
                    onMouseEnter={e => (e.currentTarget.style.background = critical ? '#FDECEA' : '#F5F4F0')}
                    onMouseLeave={e => (e.currentTarget.style.background = critical ? 'var(--mf-danger-50)' : 'transparent')}>
                    <td style={{ padding: '7px 9px', fontFamily: 'var(--mf-font-mono)', fontSize: 'var(--mf-text-sm)', color: 'var(--mf-navy-600)', fontWeight: 700, whiteSpace: 'nowrap' }}>#{row.num}</td>
                    <td style={{ padding: '7px 9px', fontSize: 'var(--mf-text-sm)', fontWeight: 500, color: 'var(--mf-ink-900)', whiteSpace: 'nowrap' }}>{row.cliente}</td>
                    <td style={{ padding: '7px 9px', fontSize: 'var(--mf-text-sm)', color: 'var(--mf-ink-500)', whiteSpace: 'nowrap' }}>{row.embarcacao}</td>
                    <td style={{ padding: '7px 9px', fontSize: 'var(--mf-text-sm)', color: 'var(--mf-ink-700)', maxWidth: 220 }}>
                    ...
                    <td style={{ padding: '7px 9px', whiteSpace: 'nowrap' }}>
                      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, background: sc.bg, color: sc.color, borderRadius: 99, padding: '2px 7px', fontSize: 11, fontWeight: 600 }}>
                        <span style={{ width: 4, height: 4, borderRadius: '50%', background: sc.dot, flexShrink: 0 }} />
                        {row.status}
                      </span>
                    </td>
                    <td style={{ padding: '7px 9px' }}>
                      <span style={{ display: 'inline-block', background: pc.bg, color: pc.color, borderRadius: 3, padding: '1px 5px', fontSize: 11, fontWeight: 600 }}>{row.prioridade}</span>
                    </td>
                    <td style={{ padding: '7px 9px', fontSize: 'var(--mf-text-sm)', color: row.tecnico === '—' ? 'var(--mf-ink-300)' : 'var(--mf-ink-600)', whiteSpace: 'nowrap' }}>
                      {row.tecnico === '—' ? <span style={{ fontStyle: 'italic' }}>Sem técnico</span> : row.tecnico}
                    </td>
                    <td style={{ padding: '7px 9px', fontFamily: 'var(--mf-font-mono)', fontSize: 'var(--mf-text-sm)', fontWeight: 600, color: 'var(--mf-ink-800)', whiteSpace: 'nowrap', textAlign: 'right' }}>
                    ...
                    <td style={{ padding: '7px 9px', fontSize: 'var(--mf-text-sm)', whiteSpace: 'nowrap', color: row.prazoAlerta ? 'var(--mf-danger-600)' : 'var(--mf-ink-400)', fontWeight: row.prazoAlerta ? 600 : 400 }}>
                      {row.prazo}
                    </td>
                    <td style={{ padding: '7px 9px' }}>
                      <button style={{ fontSize: 11, color: 'var(--mf-navy-600)', background: 'transparent', border: '1px solid var(--mf-navy-200)', borderRadius: 3, padding: '2px 7px', cursor: 'pointer', fontWeight: 500, fontFamily: 'var(--mf-font-sans)', whiteSpace: 'nowrap' }}>Ver OS</button>
                    </td>
                  </tr>
                );
              })}
          ...
          <div style={{ padding: '6px 12px', borderTop: '1px solid var(--mf-ink-100)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: V2.paper }}>
            <span style={{ fontSize: 'var(--mf-text-xs)', color: 'var(--mf-ink-400)' }}>7 de 19 registros · R$ 26.521,00 em aberto · 1 a orçar</span>
            <div style={{ display: 'flex', gap: 2 }}>
              {['‹', '1', '2', '3', '›'].map((p, i) => (
                <button key={i} style={{ width: 24, height: 22, borderRadius: 3, border: p === '1' ? '1px solid var(--mf-navy-500)' : '1px solid var(--mf-ink-200)', background: p === '1' ? 'var(--mf-navy-800)' : 'transparent', color: p === '1' ? '#fff' : 'var(--mf-ink-500)', fontSize: 'var(--mf-text-xs)', cursor: 'pointer', fontFamily: 'var(--mf-font-sans)' }}>{p}</button>
              ))}
            </div>
          </div>
        </div>
        ...
        {/* ── OS EM DETALHE ──────────────────────────────────────────────── */}
        <SectionHeader id="os-detalhe" title="OS em detalhe — #1048" sub="Azimut 42 · Marcelo Antunes · Peça aguardando fornecedor" />

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 10, marginBottom: 28 }}>
          {/* Corpo principal da OS */}
          <div style={{ background: '#fff', border: '1px solid var(--mf-ink-100)', borderRadius: 5, overflow: 'hidden' }}>
            {/* Cabeçalho */}
            <div style={{ borderLeft: '2px solid var(--mf-warning-500)', padding: '12px 16px', borderBottom: '1px solid var(--mf-ink-100)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                  <span style={{ fontFamily: 'var(--mf-font-mono)', fontSize: 'var(--mf-text-sm)', color: 'var(--mf-navy-600)', fontWeight: 700 }}>OS #1048</span>
            ...
            <div style={{ padding: '12px 16px' }}>
              <FieldLabel>Itens da OS</FieldLabel>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {/* Próxima ação */}
            <div style={{ background: '#fff', border: '1px solid var(--mf-ink-100)', borderRadius: 5, padding: '12px 14px' }}>
            ...
            <div style={{ background: '#fff', border: '1px solid var(--mf-ink-100)', borderRadius: 5, padding: '12px 14px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
                {[
                  { d: '13/05 09:14', ev: 'OS aberta — Gustavo',          dot: 'var(--mf-navy-400)'    },
                  { d: '13/05 11:30', ev: 'Diagnóstico iniciado',          dot: 'var(--mf-warning-400)' },
                  { d: '13/05 14:52', ev: 'Peça solicitada ao fornecedor', dot: 'var(--mf-ink-300)'     },
                  { d: '14/05 08:00', ev: 'WhatsApp enviado ao cliente',   dot: 'var(--mf-info-400)'    },
                ].map((ev, i, arr) => (
                  <div key={i} style={{ display: 'flex', gap: 10 }}>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flexShrink: 0 }}>
                      <div style={{ width: 7, height: 7, borderRadius: '50%', background: ev.dot, marginTop: 4 }} />
            ...
            {/* Observação interna */}
            <div style={{ background: 'var(--mf-warning-50)', border: '1px solid var(--mf-warning-100)', borderRadius: 5, padding: '10px 14px' }}>
              <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--mf-warning-600)', marginBottom: 5 }}>Observação interna</div>
        ...
        <SectionHeader id="relacionamento" title="Relacionamento — WhatsApp" sub="4 conversas ativas · 2 sem leitura · cliente respondeu há 42 min" />

        <div style={{ display: 'grid', gridTemplateColumns: '340px 1fr', gap: 10, marginBottom: 28, maxHeight: 340 }}>
          {/* Lista de conversas */}
          <div style={{ background: '#fff', border: '1px solid var(--mf-ink-100)', borderRadius: 5, overflow: 'hidden' }}>
            <div style={{ padding: '7px 12px', borderBottom: '1px solid var(--mf-ink-100)', background: V2.paper }}>
              <input placeholder="Buscar conversa…" style={{ width: '100%', height: 26, padding: '0 8px', border: '1px solid var(--mf-ink-200)', borderRadius: 3, fontSize: 'var(--mf-text-sm)', color: 'var(--mf-ink-700)', background: '#fff', outline: 'none', fontFamily: 'var(--mf-font-sans)', boxSizing: 'border-box' }} />
            </div>
            {WA_ITEMS.map((wa, i) => (
              <div key={i} style={{ padding: '9px 12px', borderBottom: i < WA_ITEMS.length - 1 ? '1px solid var(--mf-ink-50)' : undefined, display: 'flex', gap: 9, alignItems: 'flex-start', background: i === 0 ? 'var(--mf-navy-50)' : 'transparent', cursor: 'pointer' }}>
                <div style={{ width: 32, height: 32, borderRadius: '50%', background: i === 0 ? 'var(--mf-navy-200)' : 'var(--mf-ink-100)', display: 'grid', placeItems: 'center', fontSize: 11, fontWeight: 700, color: i === 0 ? 'var(--mf-navy-800)' : 'var(--mf-ink-500)', flexShrink: 0 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 1 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0 }}>
                      {!wa.lido && <span style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--mf-info-500)', display: 'inline-block' }} />}
                  ...
                  <div style={{ fontSize: 10, color: 'var(--mf-navy-400)', marginTop: 1 }}>{wa.embarcacao}</div>
                </div>
              </div>
            ))}
          </div>
          ...
          {/* Preview de conversa */}
          <div style={{ background: '#fff', border: '1px solid var(--mf-ink-100)', borderRadius: 5, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            <div style={{ padding: '9px 14px', borderBottom: '1px solid var(--mf-ink-100)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: V2.paper }}>
              <div>
            ...
            <div style={{ flex: 1, padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 7, overflowY: 'auto' }}>
              {[
            ...
            <div style={{ padding: '7px 12px', borderTop: '1px solid var(--mf-ink-100)', display: 'flex', gap: 5 }}>
              <input placeholder="Responder a Marcelo…" style={{ flex: 1, height: 30, padding: '0 10px', border: '1px solid var(--mf-ink-200)', borderRadius: 3, fontSize: 'var(--mf-text-sm)', outline: 'none', fontFamily: 'var(--mf-font-sans)' }} />
        ...
        <SectionHeader id="ui-elementos" title="Componentes — referência" sub="botões, campos, badges e estados operacionais" />

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 28 }}>
          {/* Ações */}
          <div style={{ background: '#fff', border: '1px solid var(--mf-ink-100)', borderRadius: 5, padding: '14px 16px' }}>
            <FieldLabel>Ações de OS</FieldLabel>
            ...
            <FieldLabel>Estado de envio</FieldLabel>
          ...
          {/* Campos */}
          <div style={{ background: '#fff', border: '1px solid var(--mf-ink-100)', borderRadius: 5, padding: '14px 16px' }}>
            <FieldLabel>Campos — Nova OS</FieldLabel>
          ...
          {/* Badges */}
          <div style={{ background: '#fff', border: '1px solid var(--mf-ink-100)', borderRadius: 5, padding: '14px 16px' }}>
            <FieldLabel>Status de OS</FieldLabel>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5, marginBottom: 12 }}>
              {Object.entries(STATUS_CFG).map(([label, c]) => (
                <span key={label} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, background: c.bg, color: c.color, borderRadius: 99, padding: '2px 7px', fontSize: 11, fontWeight: 600 }}>
            ...
            <div style={{ display: 'flex', gap: 5, marginBottom: 12 }}>
              {Object.entries(PRIOR_CFG).map(([p, c]) => (
                <span key={p} style={{ background: c.bg, color: c.color, borderRadius: 3, padding: '2px 7px', fontSize: 11, fontWeight: 600 }}>{p}</span>
              ))}
            </div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5, marginBottom: 12 }}>
              {[
                ...
                <span key={r.l} style={{ background: r.bg, color: r.c, borderRadius: 3, padding: '2px 7px', fontSize: 11, fontWeight: 600 }}>{r.l}</span>
              ))}
            </div>
            <FieldLabel>Empty state</FieldLabel>
            <div style={{ background: V2.paper, border: '1px solid var(--mf-ink-100)', borderRadius: 4, padding: '14px', textAlign: 'center' }}>
              <div style={{ fontSize: 'var(--mf-text-sm)', fontWeight: 600, color: 'var(--mf-ink-600)', marginBottom: 3 }}>Nenhuma OS encontrada</div>
              <div style={{ fontSize: 'var(--mf-text-xs)', color: 'var(--mf-ink-400)', marginBottom: 9 }}>Ajuste o filtro ou crie uma nova OS.</div>
              <MfButton variant="primary" size="sm">+ Nova OS</MfButton>
            </div>
          </div>
        </div>
        ...
        {/* Notificações */}
        <div style={{ background: '#fff', border: '1px solid var(--mf-ink-100)', borderRadius: 5, padding: '14px 16px', marginBottom: 28 }}>
          <FieldLabel>Notificações do sistema</FieldLabel>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 5 }}>
            {[
              ...
              return (
                <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 8, background: c.bg, borderLeft: `2px solid ${c.border}`, borderRadius: 3, padding: '7px 10px' }}>
                  <span style={{ width: 15, height: 15, borderRadius: '50%', background: c.border, color: '#fff', display: 'grid', placeItems: 'center', fontSize: 9, fontWeight: 800, flexShrink: 0, marginTop: 1 }}>{c.icon}</span>
        ...
        {/* ── TOKENS / MARCA ─────────────────────────────────────────────── */}
        <SectionHeader id="tokens" title="Tokens de design — paleta HBR Systems" sub="Navy · Gold · Ink · semânticas" />
        ...
        {/* Tipografia */}
        <div style={{ background: '#fff', border: '1px solid var(--mf-ink-100)', borderRadius: 5, padding: '14px 16px', marginBottom: 28 }}>
          <FieldLabel>Escala tipográfica — Plus Jakarta Sans</FieldLabel>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 0 }}>
            {[
              ...
              { s: 'var(--mf-text-lg)',  w: 600, ex: 'Itajaí Marine Center · Alta prioridade',             l: 'lg · 18px · 600'  },
              { s: 'var(--mf-text-md)',  w: 500, ex: 'Motor Volvo D6, revisão completa — troca de filtros.', l: 'md · 16px · 500' },
              { s: 'var(--mf-text-base)',w: 400, ex: 'Diagnóstico: falha intermitente no sistema DC 12V.',    l: 'base · 14px · 400' },
              { s: 'var(--mf-text-sm)', w: 400, ex: 'Última atualização: 14/05/2026 09:31 · Gustavo Ferreira', l: 'sm · 12px · 400' },
            ].map(t => (
              <div key={t.l} style={{ padding: '7px 0', borderBottom: '1px solid var(--mf-ink-50)', display: 'flex', alignItems: 'baseline', gap: 12 }}>
          ...
          <div style={{ marginTop: 10, background: V2.hdrBg, borderRadius: 4, padding: '8px 12px' }}>
            <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.3)', fontFamily: 'var(--mf-font-mono)', display: 'block', marginBottom: 4 }}>Mono — valores e referências</span>
            <span style={{ fontFamily: 'var(--mf-font-mono)', color: V2.gold, fontSize: 'var(--mf-text-sm)' }}>OS-1048 · R$ 8.437,00 · 2026-05-16 · PN 21414606 · #020E1A</span>
          </div>
        </div>

        {/* ── Footer HBR Systems ──────────────────────────────────────────── */}
        <div style={{ paddingTop: 16, borderTop: '1px solid var(--mf-ink-100)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 3 }}>
            <span style={{ fontWeight: 800, fontSize: 'var(--mf-text-sm)', color: 'var(--mf-navy-900)', letterSpacing: '-0.02em' }}>HBR Systems</span>
            <span style={{ fontSize: 'var(--mf-text-xs)', color: 'var(--mf-ink-300)' }}>Marineflow ERP · Design Preview V02 · Fase 1.4 · {new Date().toLocaleDateString('pt-BR')}</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
            <span style={{ fontSize: 'var(--mf-text-xs)', color: 'var(--mf-gold-600)', letterSpacing: '0.02em' }}>Energia. Controle. Confiança.</span>
            <span style={{ fontSize: 10, color: 'var(--mf-ink-300)', fontFamily: 'var(--mf-font-mono)' }}>feat/ui-design-phase-1-preview</span>
          </div>
        </div>
      </div>
...
/* ════════════════════════════════════════════════════════════════════════════
   PRIMITIVOS INTERNOS — V02
════════════════════════════════════════════════════════════════════════════ */

function SectionHeader({ id, title, sub }: { id: string; title: string; sub: string }) {
  return (
    <div id={id} style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 10, paddingBottom: 8, paddingLeft: 10, borderBottom: '1px solid var(--mf-ink-100)', borderLeft: '2px solid rgba(200,162,75,0.55)' }}>
...
function KpiCard({ label, value, detalhe, detalheColor, accent }: { label: string; value: string; detalhe: string; detalheColor: string; accent: 'danger'|'warning'|'info'|'none' }) {
  const accentColor = { danger: 'var(--mf-danger-400)', warning: 'var(--mf-warning-400)', info: 'var(--mf-info-400)', none: 'transparent' }[accent];
  return (
    <div style={{ background: '#fff', border: '1px solid var(--mf-ink-100)', borderTop: `2px solid ${accentColor}`, borderRadius: 4, padding: '11px 14px' }}>
...
function FilterChip({ children, active }: { children: React.ReactNode; active?: boolean }) {
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', height: 24, padding: '0 8px', borderRadius: 3, border: active ? '1px solid var(--mf-navy-500)' : '1px solid var(--mf-ink-200)', background: active ? 'var(--mf-navy-800)' : 'transparent', color: active ? '#fff' : 'var(--mf-ink-500)', fontSize: 11, fontWeight: 500, cursor: 'pointer', userSelect: 'none' as const, whiteSpace: 'nowrap' as const }}>
      {children}
    </span>
  );
}
...
function MfButton({ variant = 'primary', size = 'md', disabled, loading, children }: {
  ...
  const sz = { sm: { h: 27, p: '0 10px', f: 'var(--mf-text-xs)' }, md: { h: 32, p: '0 14px', f: 'var(--mf-text-sm)' }, lg: { h: 38, p: '0 18px', f: 'var(--mf-text-md)' } }[size];
  ...
  return (
    <button disabled={disabled || loading} style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 5, cursor: disabled || loading ? 'not-allowed' : 'pointer', fontFamily: 'var(--mf-font-sans)', fontWeight: 600, borderRadius: 4, outline: 'none', opacity: disabled ? 0.5 : 1, height: sz.h, padding: sz.p, fontSize: sz.f, transition: 'opacity 120ms', whiteSpace: 'nowrap', ...vars[variant] }}>
      {loading && <span style={{ width: 10, height: 10, border: '2px solid currentColor', borderTopColor: 'transparent', borderRadius: '50%', animation: 'mf-spin 0.7s linear infinite', display: 'inline-block', flexShrink: 0 }} />}
      {children}
    </button>
  );
}
...
function MfField({ label, placeholder, value, type = 'text', readOnly, disabled, error, multiline }: {
  ...
  const base: React.CSSProperties = { width: '100%', padding: '0 8px', height: 30, border: `1px solid ${error ? 'var(--mf-danger-400)' : 'var(--mf-ink-200)'}`, borderRadius: 3, fontSize: 'var(--mf-text-sm)', color: disabled ? 'var(--mf-ink-300)' : 'var(--mf-ink-800)', background: disabled ? 'var(--mf-ink-50)' : readOnly ? V2.paper : '#fff', outline: 'none', fontFamily: 'var(--mf-font-sans)', boxSizing: 'border-box' as const };
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
      <label style={{ fontSize: 11, fontWeight: 600, color: 'var(--mf-ink-600)', letterSpacing: '0.02em' }}>{label}</label>
      {multiline ? <textarea placeholder={placeholder} disabled={disabled} style={{ ...base, height: 56, padding: '6px 8px', resize: 'none' }} /> : <input type={type} placeholder={placeholder} defaultValue={value} readOnly={readOnly} disabled={disabled} style={base} />}
