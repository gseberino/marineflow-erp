...
/* ─────────────────────────────────────────────────────────────────────────────
   Marineflow ERP — Design Preview V03 (Fase 1.5)
   Híbrida curada: cabeçalho/linhas/identidade V02 + corpo operacional V01.
   Snapshot preservado. Não altere este arquivo após aprovação.
   V03 — Híbrida curada · "ERP operacional premium HBR Systems"
───────────────────────────────────────────────────────────────────────────── */

/* ── Paleta V03 (inline, não afeta tokens globais) ───────────────────────── */
const V3 = {
  hdrBg:   '#020E1A',
  paper:   '#F7F6F2',
  gold:    '#C8A24B',
...
/* ════════════════════════════════════════════════════════════════════════════
   COMPONENTE PRINCIPAL — V03
════════════════════════════════════════════════════════════════════════════ */
export default function DesignPreviewV03() {
  return (
    <div style={{ fontFamily: 'var(--mf-font-sans)', background: V3.paper, color: 'var(--mf-ink-900)', minHeight: '100vh', fontSize: 'var(--mf-text-base)', lineHeight: 1.5 }}>

      {/* ── Header HBR Systems (da V02 — preservado integralmente) ──────── */}
      <div style={{ position: 'sticky', top: 0, zIndex: 40, background: V3.hdrBg }}>
        <div style={{ height: 52, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 20px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', paddingRight: 14, borderRight: '1px solid rgba(255,255,255,0.07)', lineHeight: 1 }}>
              ...
              <span style={{ fontSize: 7, fontWeight: 700, letterSpacing: '0.22em', color: V3.gold, textTransform: 'uppercase', marginTop: 3, lineHeight: 1 }}>SYSTEMS</span>
            </div>
            <div>
              ...
              <div style={{ color: V3.goldDim, fontSize: 9, letterSpacing: '0.04em', lineHeight: 1.2, marginTop: 2 }}>Energia. Controle. Confiança.</div>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            <nav style={{ display: 'flex', gap: 1 }}>
              {NAV_SECTIONS.map(n => (
                <a key={n.id} href={`#${n.id}`}
                  ...
                  onMouseEnter={e => { const el = e.target as HTMLElement; el.style.color = V3.gold; el.style.background = 'rgba(255,255,255,0.04)'; }}
            ...
            {/* Links de versão */}
            <div style={{ display: 'flex', gap: 5, paddingLeft: 10, borderLeft: '1px solid rgba(255,255,255,0.07)' }}>
              ...
              <a href="/design-preview-v02" style={{ color: 'var(--mf-ink-600)', fontSize: 10, textDecoration: 'none', fontWeight: 500, padding: '2px 6px', borderRadius: 3, border: '1px solid rgba(255,255,255,0.06)' }}>← V02</a>
            ...
            <div style={{ display: 'flex', gap: 5 }}>
              <span style={{ background: 'rgba(200,162,75,0.12)', color: V3.gold, fontSize: 10, fontWeight: 700, padding: '2px 7px', borderRadius: 3, border: '1px solid rgba(200,162,75,0.22)', letterSpacing: '0.06em' }}>V03</span>
              <span style={{ background: 'rgba(255,255,255,0.04)', color: 'var(--mf-ink-500)', fontSize: 10, fontWeight: 600, padding: '2px 7px', borderRadius: 3, border: '1px solid rgba(255,255,255,0.07)', letterSpacing: '0.04em' }}>Fase 1.5</span>
            </div>
          </div>
        </div>

        {/* Linha fluida — acento dourado HBR Systems (da V02) */}
        <div style={{ height: 2, background: `linear-gradient(90deg, ${V3.hdrBg} 0%, rgba(200,162,75,0.25) 15%, rgba(200,162,75,0.75) 40%, #C8A24B 50%, rgba(200,162,75,0.75) 60%, rgba(200,162,75,0.25) 85%, ${V3.hdrBg} 100%)` }} />
      </div>

      {/* ── Barra de ambiente (estilo V01 — navy-950, menos escuro que V02) ── */}
      <div style={{ background: 'var(--mf-navy-950)', borderBottom: '1px solid var(--mf-navy-800)', padding: '5px 20px', display: 'flex', alignItems: 'center', gap: 8 }}>
        <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--mf-ink-500)' }}>Snapshot V03 — híbrida</span>
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '20px 20px 48px' }}>
        ...
        {/* Alertas operacionais — borda esquerda 3px da V01 (ênfase operacional) */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 32 }}>
          {ALERTAS.map((a, i) => {
            const c = ALERTA_CFG[a.tipo] ?? ALERTA_CFG.info;
            return (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, background: c.bg, border: `1px solid ${c.border}`, borderLeft: `3px solid ${c.border}`, borderRadius: 5, padding: '7px 12px' }}>
        ...
        {/* ── FILA OPERACIONAL ───────────────────────────────────────────── */}
        {/* Label da V01 — mais natural/operacional que "Controle técnico" da V02 */}
        <SectionHeader id="fila" title="Fila operacional — Ordens de Serviço" sub="19 registros · exibindo 7 · filtro: em aberto" />

        <div style={{ background: '#fff', border: '1px solid var(--mf-ink-100)', borderRadius: 6, overflow: 'hidden', marginBottom: 32 }}>
          {/* Toolbar */}
          <div style={{ padding: '8px 12px', borderBottom: '1px solid var(--mf-ink-100)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, background: V3.paper }}>
          ...
          {/* Tabela — cabeçalho navy da V02, borda crítica 3px da V01 */}
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--mf-navy-800)', background: 'var(--mf-navy-900)' }}>
                {['OS', 'Cliente', 'Embarcação', 'Serviço', 'Status', 'Prior.', 'Técnico', 'Valor', 'Prazo'].map(h => (
                  <th key={h} style={{ padding: '7px 10px', textAlign: 'left', fontSize: 10, fontWeight: 700, color: 'rgba(255,255,255,0.50)', letterSpacing: '0.07em', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>{h}</th>
                ))}
            ...
            <tbody>
              {OS_ROWS.map((row, i) => {
                ...
                return (
                  <tr key={row.num}
                    style={{ borderBottom: i < OS_ROWS.length - 1 ? '1px solid var(--mf-ink-50)' : undefined, background: critical ? 'var(--mf-danger-50)' : 'transparent', borderLeft: critical ? '3px solid var(--mf-danger-400)' : '3px solid transparent' }}
                    ...
                    <td style={{ padding: '8px 10px', fontFamily: 'var(--mf-font-mono)', fontSize: 'var(--mf-text-sm)', color: 'var(--mf-navy-600)', fontWeight: 700, whiteSpace: 'nowrap' }}>#{row.num}</td>
          ...
          <div style={{ padding: '7px 12px', borderTop: '1px solid var(--mf-ink-100)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: V3.paper }}>
        ...
        {/* ── OS EM DETALHE ──────────────────────────────────────────────── */}
        {/* Título da V01 — mais específico e operacional que a V02 */}
        <SectionHeader id="os-detalhe" title="Ordem de Serviço — #1048" sub="Marcelo Antunes · Azimut 42 · Aguardando Volvo D4 (Marine Parts BR)" />

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 12, marginBottom: 32 }}>
          {/* Corpo principal da OS */}
          <div style={{ background: '#fff', border: '1px solid var(--mf-ink-100)', borderRadius: 6, overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                ...
                {[
                  ...
                  <div key={i} style={{ display: 'flex', gap: 10 }}>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flexShrink: 0 }}>
                      <div style={{ width: 8, height: 8, borderRadius: '50%', background: ev.dot, marginTop: 4 }} />
            ...
            <div style={{ background: 'var(--mf-warning-50)', border: '1px solid var(--mf-warning-100)', borderRadius: 6, padding: '10px 14px' }}>
        ...
        {/* Label da V01 — "WhatsApp Inbox" é mais específico que simplesmente "WhatsApp" */}
        <SectionHeader id="relacionamento" title="Relacionamento — WhatsApp Inbox" sub="4 conversas ativas · 2 sem leitura · última há 42 min" />

        <div style={{ display: 'grid', gridTemplateColumns: '340px 1fr', gap: 12, marginBottom: 32, maxHeight: 340 }}>
          <div style={{ background: '#fff', border: '1px solid var(--mf-ink-100)', borderRadius: 6, overflow: 'hidden' }}>
            <div style={{ padding: '8px 12px', borderBottom: '1px solid var(--mf-ink-100)', background: V3.paper }}>
          ...
          <div style={{ background: '#fff', border: '1px solid var(--mf-ink-100)', borderRadius: 6, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            <div style={{ padding: '10px 14px', borderBottom: '1px solid var(--mf-ink-100)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: V3.paper }}>
              <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 32 }}>
          {/* Ações — ordem V02 (gold primeiro = ação primária de aprovação) */}
          <div style={{ background: '#fff', border: '1px solid var(--mf-ink-100)', borderRadius: 6, padding: '14px 16px' }}>
          ...
          <div style={{ background: '#fff', border: '1px solid var(--mf-ink-100)', borderRadius: 6, padding: '14px 16px' }}>
            ...
            <div style={{ background: V3.paper, border: '1px solid var(--mf-ink-100)', borderRadius: 5, padding: '16px', textAlign: 'center' }}>
        ...
        {/* Toasts */}
        <div style={{ background: '#fff', border: '1px solid var(--mf-ink-100)', borderRadius: 6, padding: '14px 16px', marginBottom: 32 }}>
        ...
        {/* ── TOKENS ─────────────────────────────────────────────────────── */}
        <SectionHeader id="tokens" title="Tokens de design — paleta e referência" sub="Navy · Gold · Ink · semânticas" />
        ...
        <div style={{ background: '#fff', border: '1px solid var(--mf-ink-100)', borderRadius: 6, padding: '14px 16px', marginBottom: 32 }}>
          ...
          <div style={{ marginTop: 10, background: V3.hdrBg, borderRadius: 4, padding: '8px 12px' }}>
            ...
            <span style={{ fontFamily: 'var(--mf-font-mono)', color: V3.gold, fontSize: 'var(--mf-text-sm)' }}>OS-1048 · R$ 8.437,00 · 2026-05-16 · PN 21414606 · #020E1A</span>
          </div>
        </div>

        {/* ── Footer HBR Systems (da V02) ─────────────────────────────────── */}
        <div style={{ paddingTop: 16, borderTop: '1px solid var(--mf-ink-100)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 3 }}>
            ...
            <span style={{ fontSize: 'var(--mf-text-xs)', color: 'var(--mf-ink-300)' }}>Marineflow ERP · Design Preview V03 · Fase 1.5 · {new Date().toLocaleDateString('pt-BR')}</span>
          </div>
...
/* ════════════════════════════════════════════════════════════════════════════
   PRIMITIVOS INTERNOS — V03
════════════════════════════════════════════════════════════════════════════ */

/* Da V02: gold left border confere hierarquia visual ao título da seção */
...
/* Da V02: topo colorido por severidade — comunica estado sem texto adicional */
function KpiCard({ label, value, detalhe, detalheColor, accent }: { label: string; value: string; detalhe: string; detalheColor: string; accent: 'danger'|'warning'|'info'|'none' }) {
  ...
  return (
    <div style={{ background: '#fff', border: '1px solid var(--mf-ink-100)', borderTop: `2px solid ${accentColor}`, borderRadius: 6, padding: '12px 14px' }}>
...
/* Raios e alturas da V01 (5px, 28/34/40) — botões ligeiramente mais confortáveis que V02 */
...
/* Alturas e raios da V01 — inputs 32px e 4px radius são mais confortáveis que os 30px/3px da V02 */
function MfField({ label, placeholder, value, type = 'text', readOnly, disabled, error, multiline }: {
  ...
  const base: React.CSSProperties = { width: '100%', padding: '0 8px', height: 32, border: `1px solid ${error ? 'var(--mf-danger-400)' : 'var(--mf-ink-200)'}`, borderRadius: 4, fontSize: 'var(--mf-text-sm)', color: disabled ? 'var(--mf-ink-300)' : 'var(--mf-ink-800)', background: disabled ? 'var(--mf-ink-50)' : readOnly ? V3.paper : '#fff', outline: 'none', fontFamily: 'var(--mf-font-sans)', boxSizing: 'border-box' as const };
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
