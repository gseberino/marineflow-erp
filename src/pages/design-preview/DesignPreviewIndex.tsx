...
const CURRENT: Omit<VersionCard, 'tag' | 'fase' | 'status'> = {
  name: 'Versão atual',
  desc: 'Alias para a versão mais recente aprovada. Sempre aponta para o último snapshot aceito. Atualmente aponta para V01.',
...
const STATUS_LABEL: Record<VersionCard['status'], { label: string; bg: string; color: string }> = {
  ...
  atual:        { label: 'Atual',       bg: 'var(--mf-navy-100)',    color: 'var(--mf-navy-700)'    },
  ...
  return (
    <div style={{ fontFamily: 'var(--mf-font-sans)', background: 'var(--mf-paper)', color: 'var(--mf-ink-900)', minHeight: '100vh', fontSize: 'var(--mf-text-base)', lineHeight: 1.5 }}>
      ...
      <div style={{ maxWidth: 860, margin: '0 auto', padding: '32px 20px 64px' }}>
          ...
          <div style={{ background: '#fff', border: '1px solid var(--mf-navy-200)', borderRadius: 8, padding: '16px 20px', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 20 }}>
            <div style={{ flex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                ...
                <span style={{ background: 'var(--mf-navy-100)', color: 'var(--mf-navy-700)', fontSize: 11, fontWeight: 600, padding: '1px 7px', borderRadius: 3 }}>→ V01</span>
              </div>
        ...
        {/* Versões aprovadas */}
        <div>
          <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--mf-ink-400)', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 10 }}>Snapshots aprovados</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {VERSIONS.map(v => {
              const st = STATUS_LABEL[v.status];
              return (
                <div key={v.tag} style={{ background: '#fff', border: '1px solid var(--mf-ink-100)', borderRadius: 8, padding: '16px 20px', display: 'flex', alignItems: 'flex-start', gap: 20 }}>
                  {/* Tag lateral */}
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <p style={{ margin: 0, fontSize: 'var(--mf-text-sm)', color: 'var(--mf-ink-500)', lineHeight: 1.55 }}>{v.desc}</p>
                  </div>
        ...
        <div style={{ marginTop: 24, background: 'var(--mf-ink-50)', border: '1px dashed var(--mf-ink-200)', borderRadius: 8, padding: '14px 20px' }}>
          ...
          <p style={{ margin: 0, fontSize: 'var(--mf-text-sm)', color: 'var(--mf-ink-400)', lineHeight: 1.5 }}>
            V02, V03… serão adicionadas aqui à medida que novas iterações forem aprovadas.<br />
            Cada versão preserva o estado visual exato da aprovação anterior.
          </p>
        </div>
        ...
        <div style={{ marginTop: 32, paddingTop: 16, borderTop: '1px solid var(--mf-ink-100)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ fontSize: 'var(--mf-text-xs)', color: 'var(--mf-ink-300)' }}>Marineflow ERP · Comparativo UI/UX · Fase 1.3 · {new Date().toLocaleDateString('pt-BR')}</span>
