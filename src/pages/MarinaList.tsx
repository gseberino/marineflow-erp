...
export default function MarinaList() {
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      <PageHeader title="Marinas" description="Marina directory and vessel locations">
        <Button className="gap-2 bg-accent text-accent-foreground hover:bg-accent/90"><Plus className="h-4 w-4" /> New Marina</Button>
      </PageHeader>
      <div className="relative">
        <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
        <Input placeholder="Search marinas..." value={search} onChange={e => setSearch(e.target.value)} className="pl-9" />
      </div>
          ...
          return (
            <div key={m.id} className="rounded-xl border bg-card p-5 shadow-sm hover:shadow-md transition-all">
              ...
              <div className="space-y-1.5 text-sm text-muted-foreground">
                ...
                <div className="flex items-center gap-2"><Ship className="h-3.5 w-3.5" /> {vesselCount} vessel{vesselCount !== 1 ? 's' : ''} docked</div>
              </div>
