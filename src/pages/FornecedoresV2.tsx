...
function getCadStatus(s: Supplier): CadStatus {
  const row = s as any;
  const hasContact  = !!(s.contact_name || row.phone);
  const hasLocation = !!s.city;
...
type SortKey = 'name' | 'cnpj' | 'contato' | 'cidade' | 'fone' | 'email' | 'status' | 'ativo';
type SortDir = 'asc' | 'desc';
type ColKey  = 'name' | 'cnpj' | 'contato' | 'cidade' | 'fone' | 'email' | 'status' | 'ativo' | 'acoes';

interface ColDef {
  label: string;
  weight: number;   // proportional width weight
  required: boolean;
  sortKey?: SortKey;
  align: 'left' | 'center';
}
...
const COL_DEF: Record<ColKey, ColDef> = {
  name:    { label: 'Razão Social',     weight: 24, required: true,  sortKey: 'name',    align: 'left'   },
  cnpj:    { label: 'CNPJ / CPF',       weight: 13, required: false, sortKey: 'cnpj',    align: 'left'   },
  contato: { label: 'Contato',          weight: 13, required: false, sortKey: 'contato', align: 'left'   },
  cidade:  { label: 'Cidade · UF',      weight: 12, required: false, sortKey: 'cidade',  align: 'left'   },
  fone:    { label: 'Telefone',         weight: 11, required: false, sortKey: 'fone',    align: 'left'   },
  email:   { label: 'E-mail',           weight: 16, required: false, sortKey: 'email',   align: 'left'   },
...
const COL_ORDER: ColKey[] = ['name', 'cnpj', 'contato', 'cidade', 'fone', 'email', 'status', 'ativo', 'acoes'];

// Default: name, cnpj, cidade, status, ativo, acoes (contato, fone, email initially hidden)
const DEFAULT_VISIBLE = new Set<ColKey>(['name', 'cnpj', 'cidade', 'status', 'ativo', 'acoes']);
...
export default function FornecedoresV2() {
  ...
  const ufList = useMemo(() => {
    ...
    suppliers.forEach(s => { if (s.state) set.add(s.state); });
    return [...set].sort();
  }, [suppliers]);
  ...
  const filtered = useMemo(() => {
    ...
    return suppliers.filter(s => {
      ...
      if (filterStatus !== 'all' && getCadStatus(s) !== filterStatus) return false;
      if (filterAtivo === 'ativo'   && !s.active) return false;
      if (filterAtivo === 'inativo' &&  s.active) return false;
      if (filterUF !== 'all' && (s.state ?? '') !== filterUF) return false;
      return true;
    });
  }, [suppliers, search, filterStatus, filterAtivo, filterUF]);
  ...
  const sorted = useMemo(() => {
    ...
    return [...filtered].sort((a, b) => {
      ...
      let av = '', bv = '';
      switch (sortKey) {
        case 'name':    av = a.supplier_name ?? ''; bv = b.supplier_name ?? ''; break;
        case 'cnpj':    av = a.cnpj_cpf      ?? ''; bv = b.cnpj_cpf      ?? ''; break;
  ...
  // Sort indicator per column
  const sortInd = (sk: SortKey) =>
    sortKey === sk
      ? <span style={{ color: V3.gold, marginLeft: 3, fontSize: 10, fontStyle: 'normal' }}>{sortDir === 'asc' ? '↑' : '↓'}</span>
    ...
    return (
      <div style={{ background: V3.paper, minHeight: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ textAlign: 'center', color: '#64748b', fontSize: 14 }}>
          ...
          <div>Falha ao carregar fornecedores. Tente recarregar a página.</div>
        </div>
      </div>
    );
  }
  ...
  const semContato     = suppliers.filter(s => !s.contact_name && !(s as any).phone).length;
  const semLocalizacao = suppliers.filter(s => !s.city).length;
    ...
    <div style={{ background: V3.paper, minHeight: '100%', fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <div style={{ background: V3.navy950, padding: '5px 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <span style={{ fontSize: 10, color: V3.gold, fontWeight: 700, letterSpacing: '0.13em', textTransform: 'uppercase' }}>
          Fornecedores V2 · Piloto Visual · Linguagem V03
        </span>
      ...
      <div style={{ padding: '24px 32px 40px' }}>
        ...
        <div style={{ marginBottom: 6, display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
              <h1 style={{ fontSize: 20, fontWeight: 700, color: V3.hdrBg, margin: 0, lineHeight: 1 }}>Fornecedores</h1>
            ...
            <p style={{ fontSize: 13, color: '#64748b', margin: 0 }}>
              Cadastro operacional de parceiros, peças, serviços e contatos comerciais · rota paralela · sem alteração no cadastro atual
            </p>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
            <button
              title="Criar fornecedor na tela atual"
            ...
            <button
              title="Exportação permanece na tela atual nesta fase"
              disabled
              style={{
            ...
            <button
              title="Importação permanece na tela atual nesta fase"
              onClick={() => navigate('/suppliers')}
              style={{
                height: 34, padding: '0 14px', fontSize: 13, fontWeight: 500,
                color: '#475569', background: '#fff', border: '1px solid #d1d5db',
        ...
        <div style={{ marginBottom: 20, textAlign: 'right', fontSize: 11, color: '#94a3b8', fontStyle: 'italic' }}>
          Criação, importação e exportação permanecem na tela atual nesta fase.
        </div>

        {/* ── KPI row ── */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16, marginBottom: 24 }}>
          <KpiCard label="Total de fornecedores" value={total}          accent="none" />
          <KpiCard label="Ativos"                value={ativos}         sub={`${total - ativos} inativos`} accent="ok" />
        ...
        <div style={{ marginBottom: 14, display: 'flex', flexWrap: 'wrap', gap: 8, alignItems: 'center' }}>
          ...
          <div style={{ position: 'relative', flex: '1 1 220px', maxWidth: 360 }}>
            ...
            <input
              ...
              placeholder="Nome, fantasia, CNPJ, contato, cidade…"
              ...
              <>
                ...
                <div style={{ position: 'absolute', top: 'calc(100% + 4px)', right: 0, zIndex: 50, background: '#fff', border: '1px solid #e2e8f0', borderRadius: 6, padding: '6px 0', minWidth: 196, boxShadow: '0 4px 16px rgba(0,0,0,0.10)' }}>
                  ...
                  <div style={{ padding: '8px 14px 2px', fontSize: 10, color: '#94a3b8', borderTop: '1px solid #f0efeb', marginTop: 4 }}>
                    Razão Social e Ações são obrigatórias.
                  </div>
                </div>
              </>
            )}
          ...
          <span style={{ fontSize: 12, color: '#94a3b8', whiteSpace: 'nowrap' }}>
            {sorted.length === 0
              ? 'Nenhum resultado'
              : `${rangeStart}–${rangeEnd} de ${sorted.length} fornecedor${sorted.length !== 1 ? 'es' : ''}`
            }
          </span>
        </div>

        {/* ── Table ── */}
        <SectionHeader>Cadastro de Fornecedores e Parceiros Comerciais</SectionHeader>
        <div style={{ background: '#fff', borderRadius: 6, border: '1px solid #e2e8e0', overflow: 'hidden' }}>
          {sorted.length === 0 ? (
            <div style={{ padding: '48px 20px', textAlign: 'center' }}>
              <div style={{ fontSize: 13, color: '#94a3b8', marginBottom: 12 }}>
                {hasActiveFilters ? 'Nenhum fornecedor encontrado com os filtros atuais.' : 'Nenhum fornecedor cadastrado.'}
              </div>
              {hasActiveFilters && (
            ...
            /* overflowX: auto handles narrow viewports; table fills 100% on wide ones */
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', minWidth: 580, tableLayout: 'fixed', borderCollapse: 'collapse', fontSize: 13 }}>
                <colgroup>
                      ...
                      >
                        {visibleColList.map((k, ci): React.ReactNode => {
                          ...
                          switch (k) {
                            ...
                            case 'cnpj':
                              return (
                                <td key={k} style={{ ...base, fontFamily: 'monospace', fontSize: 11, color: '#64748b', letterSpacing: '0.03em' }}>
                                  {s.cnpj_cpf || <span style={{ color: '#cbd5e1', fontFamily: 'inherit' }}>—</span>}
                                </td>
                              );
                            case 'contato':
                              return (
                                <td key={k} style={{ ...base, color: '#475569' }}>
                                  {s.contact_name || <span style={{ color: '#cbd5e1' }}>—</span>}
                                </td>
                              );
                            case 'cidade':
                              return (
                            ...
                            case 'acoes':
                              return (
                                <td key={k} style={base}>
                                  <button
                                    title="Editar cadastro na tela atual de Fornecedores"
                                    onClick={() => navigate('/suppliers')}
                                    style={{
