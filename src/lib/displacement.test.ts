// Cobertura do cálculo de deslocamento.
//
...
//
// `calculateDisplacement` consulta `app_settings`; o bloco final a testa com o cliente mockado,
// inclusive a haversine e o × 2 da ida e volta — trocar o raio da Terra, inverter lat/lng ou
// perder o × 2 iria direto para travel_distance_km e travel_cost_total da OS.
...
describe('calculateTravelCost — a conta que vai para a OS', () => {
  ...
  // [NOVO-024a] A tabela vai até 3 técnicos; o código fazia `rates.hourly[4] || rates.hourly[1]`
  // e caía na tarifa de UM. Quatro pessoas na estrada custavam menos que três, e o número de
  // técnicos é campo livre na tela.
  it('[NOVO-024] 4 técnicos custam MAIS que 3 — o passo da tabela é extrapolado, nunca a tarifa de 1', () => {
    const params = { distance_km: 0, travel_hours: 1, ferry_cost: 0, travel_type: 'comercial' as const };
    ...
    // Política PROVISÓRIA (em produção desde 12/08): acima da última faixa, o passo da
    // própria tabela (250 − 170 = 80) por técnico adicional. Quanto cobrar de verdade é
    // decisão comercial do dono; o que não pode é 4 custar como 1.
    expect(quatro).toBe(330);
    expect(quatro).toBeGreaterThan(tres);
  });

  it('[NOVO-024] acima do teto nunca cai: de 1 a 6 técnicos o custo é não-decrescente', () => {
    const params = { distance_km: 0, travel_hours: 1, ferry_cost: 0, travel_type: 'comercial' as const };
    ...
    for (let i = 1; i < custos.length; i++) {
      expect(custos[i], `${i + 1} técnicos não pode custar menos que ${i}`)
        .toBeGreaterThanOrEqual(custos[i - 1]);
    }
    expect(custos).toEqual([90, 170, 250, 330, 410, 490]);
  });

  it('[NOVO-024] o passo vem da configuração da empresa, não de constante no código', () => {
    const rates = travelRatesFromSettings({
  ...
  it('[NOVO-024] contagem zerada ou negativa vira 1 técnico — alguém sempre vai', () => {
    const params = { distance_km: 0, travel_hours: 1, ferry_cost: 0, travel_type: 'comercial' as const };
    expect(calculateTravelCost({ ...params, technician_count: 0 })).toBe(90);
    expect(calculateTravelCost({ ...params, technician_count: -2 })).toBe(90);
  });

  it('[NOVO-024] hourlyRateFor sozinha: faixa exata, teto e borda', () => {
    ...
    // Uma faixa só não descreve passo: é o teto, e vale para qualquer contagem.
    const umaFaixa = { ...DEFAULT_TRAVEL_RATES, hourly: { 1: 100 } };
    expect(hourlyRateFor(umaFaixa, 3)).toBe(100);
    // Buraco na tabela (só 1 e 3): 2 técnicos sobem para a faixa seguinte, não descem para 1;
    // acima do teto o passo é por técnico ((300 − 100) / 2 = 100), então 4 → 400.
    const buraco = { ...DEFAULT_TRAVEL_RATES, hourly: { 1: 100, 3: 300 } };
    expect(hourlyRateFor(buraco, 2)).toBe(300);
    expect(hourlyRateFor(buraco, 4)).toBe(400);
  });

  it('[NOVO-024] faixa configurada em zero é zero — o `||` antigo trocava de faixa', () => {
    // travelRatesFromSettings rejeita 0, então isto só chega por objeto montado à mão — mas a
    // função não pode decidir sozinha que 0 "não vale".
    const rates = { ...DEFAULT_TRAVEL_RATES, hourly: { 1: 90, 2: 0, 3: 250 } };
    expect(hourlyRateFor(rates, 2)).toBe(0);
  });
});

describe('[NOVO-024b] calculateDisplacement usa a tarifa configurada, não a do código', () => {
  ...
  it('sem `rates`, devolve o km da configuração — não o 1.10 que estava fixo no return', async () => {
  ...
  it('o total usa a tarifa por km configurada, sobre ida E volta', async () => {
    const { calculateDisplacement } = await import('./displacement');
    const r = await calculateDisplacement(-27.0, -48.6728, 1);
    // 0,0922° de latitude ≈ 10,25 km; ida e volta ≈ 20,5. Pina o raio da Terra e o × 2.
    expect(r.distance_km).toBeCloseTo(20.5, 1);
  ...
  it('com `rates` passadas, usa-as e devolve o km DELAS — mesmo que app_settings diga outro', async () => {
    // É o caminho do formulário da OS: ele já tem as tarifas na mão e precisa gravar na OS o
    // mesmo km que usou na conta, sem depender de uma segunda leitura da tabela.
    const { calculateDisplacement } = await import('./displacement');
    const doFormulario = { ...DEFAULT_TRAVEL_RATES, km_rate: 3.5 };
    const r = await calculateDisplacement(-27.0, -48.6728, 1, doFormulario);
    expect(r.cost_per_km).toBe(3.5);
    expect(r.total_cost).toBeCloseTo(r.distance_km * 3.5, 2);
    expect(r.total_cost).not.toBeCloseTo(r.distance_km * 2, 2);
  });
});
