...
export function WhatsAppWebhookValidator() {
  ...
  const copyUrl = async () => {
    ...
    toast({ title: 'URL copiada', description: 'Cole no painel do provider em "On Message Received".' });
  };
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <Activity className="h-4 w-4" /> Webhook WhatsApp
        </CardTitle>
        <CardDescription>
          Verifique se mensagens estão chegando ao sistema. Use o monitoramento ao vivo enquanto envia
          uma mensagem de teste para o número conectado.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="space-y-2">
          <div className="text-xs font-medium text-muted-foreground">URL do Webhook</div>
          ...
          <>
            ...
            {data.health_status === 'never' && (
              <Alert variant="destructive">
                ...
                <AlertDescription>
                  O webhook não recebeu tráfego ainda. No painel do Evolution Manager, configure o webhook
                  com a URL acima. Em seguida, envie uma mensagem de teste do seu celular pessoal para o
                  número conectado e clique em "Validar webhook agora".
                </AlertDescription>
              </Alert>
            )}
            ...
            {data.health_status === 'stale' && (
              <Alert>
                ...
                <AlertDescription>
                  ...
                  acabou de enviar e não chegou, verifique se a URL no painel do Evolution Manager está correta.
                </AlertDescription>
              </Alert>
            )}
